    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      {catData.length === 0 ? (
        ...
        <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }} className="chart-row">
          ...
          <div style={{ flex: "1 1 340px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "1.25rem" }}>
            <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 16, fontWeight: 600, color: T.text, marginBottom: 4 }}>Amount by Category</div>
            <div style={{ fontSize: 12, color: T.textSm, marginBottom: "1rem" }}>Absolute spend comparison</div>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={catData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false} />
                <XAxis dataKey="name" tick={{ fontSize: 11, fill: T.textSm }} axisLine={false} tickLine={false} />
                ...
                <Bar dataKey="value" radius={[5, 5, 0, 0]}>
                  {catData.map((entry, i) => <Cell key={i} fill={CAT_COLORS[entry.name] || T.textSm} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
